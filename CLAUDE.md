...
## Architecture
**Next.js App Router** with a sidebar-driven layout. All pages live under `src/app/`. Components in `src/components/`. Path alias: `@/*` → `src/*`.

### Data layer
Four persistence patterns coexist:
1. **File-based JSON** (`data/`) — Priorities, suggestions, weekly reports. API routes in `src/app/api/priorities/route.ts` do read/write against these files.
2. **Browser localStorage** — Quiz history, learner profile, lab history, notes, scenario history. Client components own this state.
3. **External APIs** — Claude (Anthropic SDK, Sonnet 4) for quiz generation, lab challenges, grading, and workflow building. Gmail + Attio via Python scripts.
4. **Supabase (PostgreSQL)** — Platform database for all structured data. Project: Amitis Master Dashboard (https://njmqygpadjqlnbinblun.supabase.co).
...
1. Skill created/modified on Cowork, Claude Code, or Claude.ai
2. skill-analytics (Mode 3) or `sync-skills.py` writes to `skill_proposals` with type='submission', status='pending_review'
3. Admin reviews in Skills Hub Development tab → approves or rejects
4. Approved skills move to `skill_catalog` via POST `/api/skills/approve`
...
|---|---|---|
| `/` | Active | Dashboard overview + priority board |
| `/priorities` | Active | AI-ranked Kanban (drag-drop via @hello-pangea/dnd) |
| `/skills` | Active | Skills Hub — Skills In Use, Marketplace (deep detail + embedded Claude), Development (submissions + proposals), Eval History |
| `/portfolio/fund-returns` | Active | Iframes the separate fund-returns-dashboard (port 5050) |
| `/operations/enablement` | Active | Tabbed: Daily Quiz, Architecture Lab, Notes, Weekly Reports |
| `/operations/ai-initiatives` | Active | AI initiative tracker — clickable cards with detail views, status progression, linked skills |
| `/portfolio/fund-accounting` | Scoping | Supabase schema designed, tables created. Next: wire dashboard to read from Supabase, build reconciliation dot. |
| `/investor-relations` | Coming soon | One-pagers, newsletters, LP materials, X post scheduling |
| `/research` | Coming soon | Fund vetting, deal evaluation |
| `/acio` | Coming soon | ACIO deal pipeline (under_review/invested/passed), investment memos |
...
All AI-powered routes use Claude Sonnet 4 via `@anthropic-ai/sdk`. Several use Claude's `web_search` tool for real-time data.
- `priorities/` — GET/POST for `data/priorities.json`
- `quiz/` — generate, grade, readings, study-guide, tips
- `lab/` — generate, flash, diagram, grade, stress-test, workflow
- `skills/` — proposals (GET), approve (POST), reject (POST), bulk-approve (POST), analyze (POST), content (GET)
...
- When explaining technical concepts to Gage, offer to log them via the learning-log skill

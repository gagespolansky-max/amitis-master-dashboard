...
describe("processThread idempotency", () => {
  ...
  it("creates an interaction, org, people, and action item on first run", async () => {
  ...
  it("re-running with a new source_id but same thread_id continues the existing action item", async () => {
    await processThread(BASE_INPUT)
  ...
  it("low-confidence action items get the low_confidence tag", async () => {
    ...
    expect(tagRows.map((t) => t.tag)).toContain("low_confidence")
  })

  it("matches existing organizations by domain case-insensitively on re-run", async () => {
    await processThread(BASE_INPUT)
    await processThread({
      ...BASE_INPUT,
      source_id: "msg-3",

...
import { aiSystemsQuestions, claudeCodeQuestions, type QuizQuestion } from '@/lib/quiz-data'
...
export default function QuizPortal() {
  ...
  async function generateStudyGuide() {
    ...
    setShowGuide(true)
    try {
      const resp = await fetch('/api/quiz/study-guide', {
        method: 'POST',
  ...
  async function fetchReadings() {
    ...
    try {
      const resp = await fetch('/api/quiz/readings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function finishQuiz() {
    ...
    if (uniqueWeak.length > 0) {
      setLoadingTips(true)
      try {
        const resp = await fetch('/api/quiz/tips', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },

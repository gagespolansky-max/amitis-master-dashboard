...
import { Deal, DealStage, DealPriority, STAGES, STAGE_LABELS } from "../_lib/types"
...
export default function ACIOBoard() {
  ...
  const fetchDeals = useCallback(async () => {
    const res = await fetch("/acio/api/deals")
  ...
  async function handleScan() {
    setScanning(true)
    try {
      const res = await fetch("/acio/api/scan", {
        method: "POST",
  ...
  async function handleDragEnd(result: DropResult) {
    ...
    const res = await fetch(`/acio/api/deals/${dealId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleBulkConfirm(ids: string[], overrides?: Record<string, { deal_type?: string; stage?: DealStage; priority?: DealPriority }>) {
    await Promise.all(
      ids.map((id) => {
        ...
        return fetch(`/acio/api/deals/${id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  async function handleMerge(targetId: string, sourceId: string) {
    const res = await fetch(`/acio/api/deals/${targetId}/merge`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },

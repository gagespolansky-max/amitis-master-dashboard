...
export default function DealPanel({ deal, onClose, onUpdate, onDelete, onMerge }: DealPanelProps) {
  ...
  useEffect(() => {
    fetch(`/acio/api/deals/${deal.id}/emails`)
      .then((r) => r.json())
      .then(setEmails)
      .catch(() => {})
  }, [deal.id])
  ...
  async function patchDeal(fields: Record<string, unknown>) {
    const res = await fetch(`/acio/api/deals/${deal.id}`, {
      method: "PATCH",
  ...
  async function uploadMemo(file: File) {
    ...
    const res = await fetch(`/acio/api/deals/${deal.id}/memo`, {
      method: "POST",
      body: formData,
    })
  ...
  async function handleDelete() {
    ...
    await fetch(`/acio/api/deals/${deal.id}`, { method: "DELETE" })
    onDelete(deal.id)
  }
  ...
  async function handleEnrich() {
    setEnriching(true)
    try {
      const res = await fetch(`/acio/api/deals/${deal.id}/enrich`, { method: "POST" })
      if (res.ok) {
        const updated = await res.json()

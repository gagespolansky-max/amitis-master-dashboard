...
export default function ACIOBoard() {
  ...
  async function handleScan() {
    setScanning(true)
    try {
      const res = await fetch("/acio/deals/api/scan", {
        method: "POST",
  ...
  async function handleDragEnd(result: DropResult) {
    ...
    const res = await fetch(`/acio/deals/api/${dealId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleBulkConfirm(ids: string[], overrides?: Record<string, { deal_type?: string; stage?: DealStage; priority?: DealPriority }>) {
    await Promise.all(
      ids.map((id) => {
        ...
        return fetch(`/acio/deals/api/${id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  async function handleMerge(targetId: string, sourceId: string) {
    const res = await fetch(`/acio/deals/api/${targetId}/merge`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
